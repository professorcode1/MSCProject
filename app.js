  ...
  if (req.isAuthenticated()) {
    ...
    if (!req.user.bonusQuestionsVisible) {
      ...
      if (bonusQuestionUnlocked)
        await User.updateOne({ _id: req.user._id }, { bonusQuestionsVisible: true });
    }
...
app.get("/teamanswerhistory", async (req, res) => {
  if (req.isAuthenticated()) {
    if (req.user.adminPrivilege && req.query.teamid) {
      ...
      if (! await User.exists({ _id: mongoose.Types.ObjectId(req.query.teamid) }))
        return res.send("user doesn't exists");

      const answers = new Array(0);
      const { answersArray: teamAnswerIds } = await User.findById(req.query.teamid);
